        ...
        *has_return_value = true;
        ...
        if (validate_return_type && ptlang_rc_deref(statement).content.exp != NULL)
        {
            ptlang_verify_make_and_check_implicit_cast(
                &ptlang_rc_deref(statement).content.exp, wanted_return_type,
                ptlang_rc_deref(ptlang_rc_deref(statement).content.exp).pos, ctx, errors);
        }
        break;
...
static void ptlang_verify_exp(ptlang_ast_exp exp, ptlang_context *ctx, ptlang_error **errors)
{
    ...
    case PTLANG_AST_EXP_ASSIGNMENT:
    {
        ...
        ptlang_verify_make_and_check_implicit_cast(&ptlang_rc_deref(exp).content.binary_operator.right_value,
                                                   ptlang_rc_deref(left).ast_type, ptlang_rc_deref(exp).pos,
                                                   ctx, errors);
        break;
    }
    ...
    case PTLANG_AST_EXP_REMAINDER:
    {
        ...
        {
            ptlang_verify_make_implicit_cast(&ptlang_rc_deref(exp).content.binary_operator.left_value,
                                             ptlang_rc_deref(exp).ast_type, ptlang_rc_deref(exp).pos);
            ptlang_verify_make_implicit_cast(&ptlang_rc_deref(exp).content.binary_operator.right_value,
                                             ptlang_rc_deref(exp).ast_type, ptlang_rc_deref(exp).pos);
        }
        break;
    }
    ...
    case PTLANG_AST_EXP_NOT_EQUAL:
    {
        ...
        {
            size_t message_len =
                sizeof("The operant structs must either be both numbers or both references.");
            char *message = ptlang_malloc(message_len);
            memcpy(message, "The operant structs must either be both numbers or both references.",
                   message_len);
            arrput(*errors, ((ptlang_error){
                                .type = PTLANG_ERROR_TYPE,
                                .pos = ptlang_rc_deref(ptlang_rc_deref(exp).pos),
    ...
    {
        ptlang_verify_exp(unary, ctx, errors);
        ptlang_rc_deref(exp).ast_type = ptlang_ast_type_integer(true, ctx->pointer_bytes * 8, NULL);
        break;
    }
    ...
    case PTLANG_AST_EXP_STRUCT:
    {
        ...
        for (size_t i = 0; i < arrlenu(ptlang_rc_deref(exp).content.struct_.members); i++)
        {
            ...
            //     member_type_in_struct_def, ptlang_rc_deref(exp).content.struct_.members[i].pos, ctx,
            //     errors);
    ...
    case PTLANG_AST_EXP_ARRAY:
    {
        ...
        if (member_type != NULL)
        {
            ...
            for (size_t i = 0; i < arrlenu(ptlang_rc_deref(exp).content.array.values); i++)
            {
                ...
                ptlang_verify_make_and_check_implicit_cast(
                    &ptlang_rc_deref(exp).content.array.values[i], member_type,
                    ptlang_rc_deref(ptlang_rc_deref(exp).content.array.values[i]).pos, ctx, errors);
                                      ...
                                      ptlang_error **errors)
{
    ...
    for (size_t i = 0; i < arrlenu(struct_defs); i++)
    {
        arrput(nodes, (struct node_s){.index = -1});
    }
    for (size_t i = 0; i < arrlenu(struct_defs); i++)
    {
        ...
        {
            arrput(nodes[i].edges_to, &nodes[shget(ctx->type_scope, referenced_types[j]).index]);
        }
    ...
    for (size_t i = 0, lowlink = 0; i < arrlenu(cycles);)
    {
        ...
        {
            shget(ctx->type_scope, ptlang_rc_deref(struct_defs[cycles[j] - nodes]).name.name)
                .value.struct_def = NULL;
            arrput(message_components, ptlang_rc_deref(struct_defs[cycles[j] - nodes]).name.name);
            arrput(message_components, ", ");
        }
        ...
        if (j == i + 1)
        {
            ...
            arrput(message_components, " is recursive.");
        }
        else
        {
            arrins(message_components, 0, "The structs ");
            arrput(message_components, " are recursive.");
        }
                                             ...
                                             ptlang_error **errors)
{
    ...
    for (size_t i = 0; i < arrlenu(ptlang_rc_deref(ast_module).type_aliases); i++)
    {
        ...
        for (size_t j = 0; j < arrlenu(referenced_types); j++)
        {
            arrput(nodes[i].edges_to, &nodes[referenced_types[j]]);
        }
    ...
    for (size_t i = 0, lowlink = 0; i < arrlenu(cycles);)
    {
        ...
        for (j = i; j < arrlenu(cycles) && lowlink == cycles[j]->lowlink; j++)
        {
            arrput(message_components, ptlang_rc_deref(ast_module).type_aliases[cycles[j] - nodes].name.name);
            arrput(message_components, ", ");
        }
        arrpop(message_components);
        ...
        if (j == i + 1)
        {
            ...
            arrput(message_components, " contains a self reference.");
        }
        else
        {
            arrins(message_components, 0, "The type aliases ");
            arrput(message_components, " form a circular definition.");
        }
                                         ...
                                         ptlang_context *ctx, ptlang_error **errors)
{
    ...
    if (eval_mode >= PTLANG_VERIFY_EVAL_FULLY)
    {
        ...
        if (substituted != NULL && evaluated == NULL)
...
ptlang_ast_exp ptlang_verify_get_default_value(ptlang_ast_type type, ptlang_context *ctx)
{
    ...
    case PTLANG_AST_TYPE_ARRAY:
    {
        ...
        for (size_t i = 0; i < ptlang_rc_deref(type).content.array.len; i++)
        {
            ...
            arrput(array_values, element_default);
        }
        ptlang_rc_deref(default_value) = (struct ptlang_ast_exp_s){
                                      ...
                                      ptlang_context_type_scope *type_scope)
{
    ...
    *node_info = (ptlang_verify_node_info){
    ...
    ptlang_ast_type type = ptlang_context_unname_type(ptlang_rc_deref(path_exp).ast_type, type_scope);
    ...
    case PTLANG_AST_TYPE_ARRAY:
    {
        for (size_t i = 0; i < ptlang_rc_deref(type).content.array.len; i++)
        {
            char *index_str_repr = ptlang_malloc(sizeof("18446744073709551615"));
            snprintf(index_str_repr, sizeof("18446744073709551615"), "%" PRIu64, i);
            ...
            ptlang_rc_deref(array_element).ast_type =
                ptlang_rc_add_ref(ptlang_rc_deref(type).content.array.type);
    ...
    case PTLANG_AST_TYPE_NAMED:
    {
        ...
        for (size_t i = 0; i < arrlenu(ptlang_rc_deref(struct_def).members); i++)
        {
            ...
            ptlang_rc_deref(struct_member).ast_type =
                ptlang_rc_deref(ptlang_rc_deref(struct_def).members[i]).type == NULL
                    ? NULL
                                                      ...
                                                      ptlang_error **errors)
{
    ...
    for (size_t i = 0, lowlink = 0; i < arrlenu(cycles);)
    {
        ...
        for (j = i; j < arrlenu(cycles) && lowlink == cycles[j]->lowlink; j++)
        {
            ...
            arrput(message_components, CONST_STR(", "));
        }
        arrpop(message_components);
        ...
        if (j == i + 1)
        {
            ...
            arrput(message_components, CONST_STR(" refers to itself."));
        }
        else
        {
            arrins(message_components, 0, CONST_STR("The global variables "));
            arrput(message_components, CONST_STR(" form a circular definition."));
        }
...
static void ptlang_verify_set_init(ptlang_verify_node_info *node_info, ptlang_ast_exp init,
                                   ptlang_context *ctx)
{
    if (ptlang_rc_deref(node_info->name).ast_type != NULL &&
        ptlang_rc_deref(ptlang_rc_deref(node_info->name).ast_type).type == PTLANG_AST_TYPE_ARRAY)
    {
        ...
        while (arrlenu(ptlang_rc_deref(init).content.array.values) <
               ptlang_rc_deref(ptlang_rc_deref(node_info->name).ast_type).content.array.len)
        {
            arrput(ptlang_rc_deref(init).content.array.values,
                   ptlang_verify_get_default_value(
...
static void ptlang_verify_make_and_check_implicit_cast(ptlang_ast_exp *exp, ptlang_ast_type type,
                                                       ...
                                                       ptlang_error **errors)
{
    ptlang_verify_check_implicit_cast(ptlang_rc_deref(*exp).ast_type, type, pos, ctx, errors);
    ptlang_verify_make_implicit_cast(exp, type, pos);
}

static void ptlang_verify_make_implicit_cast(ptlang_ast_exp *exp, ptlang_ast_type type,
                                             ptlang_ast_code_position pos)
{
    *exp = ptlang_ast_exp_cast_new(ptlang_rc_add_ref(type), *exp, ptlang_rc_add_ref(pos));
    ptlang_rc_deref(*exp).ast_type = ptlang_rc_add_ref(type);
}
