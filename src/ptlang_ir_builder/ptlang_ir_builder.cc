...
extern "C"
{
    ...
    static void ptlang_ir_builder_module(ptlang_ast_module module, ptlang_ir_builder_context *ctx)
    {
        ...
        for (size_t i = 0; i < arrlenu(ptlang_rc_deref(module).functions); i++)
        {
            if (ptlang_rc_deref(ptlang_rc_deref(module).functions[i]).stmt != NULL)
            {
    ...
    static llvm::Function *ptlang_ir_builder_func_decl(ptlang_ast_func func, ptlang_ir_builder_context *ctx)
    {
        ...
        llvm::Function *function =
            llvm::Function::Create(function_type,
                                   (ptlang_rc_deref(func).exported || ptlang_rc_deref(func).stmt == NULL)
    ...
    static llvm::DIType *ptlang_ir_builder_di_type(ptlang_ast_type ast_type, ptlang_ir_builder_context *ctx)
    {
        ...
        case ptlang_ast_type_s::PTLANG_AST_TYPE_FUNCTION:
            ret_type = llvm::DIDerivedType::get(
                ...
                std::nullopt, std::nullopt, llvm::DINode::FlagZero);
            break;
        ...
        case ptlang_ast_type_s::PTLANG_AST_TYPE_HEAP_ARRAY:
        {
            ...
            elements[0] = llvm::DIDerivedType::get(ctx->llvm_ctx, llvm::dwarf::DW_TAG_member, "len", NULL, 0,
                                                   ...
                                                   0, std::nullopt, std::nullopt, llvm::DINode::FlagZero);
            ...
            llvm::DIType *ptr_type = llvm::DIDerivedType::get(
                ...
                std::nullopt, std::nullopt, llvm::DINode::FlagZero);
            elements[1] = llvm::DIDerivedType::get(ctx->llvm_ctx, llvm::dwarf::DW_TAG_member, "ptr", NULL, 0,
                                                   heap_array, ptr_type, 0, 0, 0, std::nullopt, std::nullopt,
                                                   llvm::DINode::FlagZero);
            ...
            if (entry.type == ptlang_context_type_scope_entry_s::PTLANG_CONTEXT_TYPE_SCOPE_ENTRY_STRUCT)
            {
                ...
                for (size_t i = 0; i < arrlenu(ptlang_rc_deref(entry.value.struct_def).members); i++)
                {
                    elements[i] = llvm::DIDerivedType::get(
                        ...
                        0, 0, 0, std::nullopt, std::nullopt, llvm::DINode::FlagZero);
                }
    ...
    static llvm::Constant *ptlang_ir_builder_exp_const(ptlang_ast_exp exp, ptlang_ir_builder_context *ctx)
    {
        ...
        case ptlang_ast_exp_s::PTLANG_AST_EXP_INTEGER:
        {
            ...
            char *number = ptlang_rc_deref(exp).content.str_prepresentation;
            uint8_t radix = 10;
            ...
            return llvm::ConstantInt::get((llvm::IntegerType *)type,
                                          llvm::StringRef(number, size_str - number), radix);
        }
        ...
        case ptlang_ast_exp_s::PTLANG_AST_EXP_BINARY:
        {
            ...
            llvm::Constant *byte = llvm::ConstantInt::get(type, 256, false);

            for (uint32_t i = 0; i < byte_size; i++)
            {
                if (i != 0)
                    value = llvm::ConstantExpr::getNUWMul(value, byte);
    ...
    llvm::Value *ptlang_ir_builder_exp(ptlang_ast_exp exp, ptlang_ir_builder_fun_ctx *ctx)
    {
        ...
        case ptlang_ast_exp_s::PTLANG_AST_EXP_NEGATION:
        {
            ...
            {
                bool is_signed = ptlang_rc_deref(ptlang_rc_deref(exp).ast_type).content.integer.is_signed;
                return ctx->ctx->builder.CreateNeg(operand, "neg", is_signed);
            }
            else
            { // PTLANG_AST_TYPE_FLOAT
    ...
    static llvm::Value *ptlang_ir_builder_exp_ptr(ptlang_ast_exp exp, ptlang_ir_builder_fun_ctx *ctx)
    {
        ...
        case ptlang_ast_exp_s::PTLANG_AST_EXP_ARRAY_ELEMENT:
        {
            ...
            llvm::BasicBlock *no_overflow_block =
                llvm::BasicBlock::Create(ctx->ctx->llvm_ctx, "no_overflow", ctx->func);

            llvm::BasicBlock *old_block = ctx->ctx->builder.GetInsertBlock();
                    ...
                    .type == ptlang_ast_type_s::PTLANG_AST_TYPE_ARRAY)
            {
                ...
                index = ptlang_ir_builder_exp(ptlang_rc_deref(exp).content.array_element.index, ctx);

                length = llvm::ConstantInt::get(
                    ctx->ctx->integer_ptrsize_type,
                    ptlang_rc_deref(
                        ptlang_rc_deref(ptlang_rc_deref(exp).content.array_element.array).ast_type)
                        .content.array.len);

                ctx->ctx->builder.SetInsertPoint(no_overflow_block);

                exp_ptr = ctx->ctx->builder.CreateInBoundsGEP(
            ...
            else
            {
                ...
                llvm::Value *heap_array_struct =
                    ptlang_ir_builder_exp(ptlang_rc_deref(exp).content.array_element.array, ctx);
                index = ptlang_ir_builder_exp(ptlang_rc_deref(exp).content.array_element.index, ctx);

                llvm::Value *heap_array_ptr =
                    ctx->ctx->builder.CreateExtractValue(heap_array_struct, 0, "arrayelementextractptr");
                length =
                    ctx->ctx->builder.CreateExtractValue(heap_array_struct, 1, "arrayelementextractlength");

                ctx->ctx->builder.SetInsertPoint(no_overflow_block);

                llvm::Type *elem_type = ptlang_ir_builder_type(ptlang_rc_deref(exp).ast_type, ctx->ctx);

                exp_ptr = ctx->ctx->builder.CreateInBoundsGEP(elem_type, heap_array_ptr,
                                                              llvm::ArrayRef(index), "arrayelementgep");
            }
            ctx->ctx->builder.SetInsertPoint(old_block);

            llvm::Value *index_as_ptrsizeint =
                ctx->ctx->builder.CreateZExtOrTrunc(index, ctx->ctx->integer_ptrsize_type, "outofboundscast");

            llvm::Value *is_in_bounds = ctx->ctx->builder.CreateCmp(
                llvm::ICmpInst::ICMP_ULT, index_as_ptrsizeint, length, "outofboundscmp");

            llvm::BasicBlock *overflow_block =
                llvm::BasicBlock::Create(ctx->ctx->llvm_ctx, "overflow", ctx->func);

            // ctx->ctx->builder
            //     .CreateAssumption()

            ctx->ctx->builder.CreateCondBr(is_in_bounds, no_overflow_block, overflow_block);

            ctx->ctx->builder.SetInsertPoint(overflow_block);
            ctx->ctx->builder.CreateUnreachable();
